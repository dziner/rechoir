...
- [ ] P1 데이터 소스 범위 결정: `할렐루야`처럼 지정 플레이리스트에 없는 과거 공연, 전체 예배 영상, 외부 기록을 수동 공연 기록으로 보강할지 결정
...
- [x] done P1 2026-06-29 추천 필터 2부예배 여부 추가: `derived.has2nd` 기준으로 2부예배 찬양 경험이 있는 곡/없는 곡을 필터링
- [x] done P1 2026-06-29 `할렐루야` 수집 현황 점검: 라이브 Sheet와 공개 YouTube 플레이리스트 183개 제목 기준으로 `할렐루야` 계열 항목이 1개뿐임을 확인
- [x] done P2 2026-06-29 현악기 태그 표시 정리: 기본값인 `현악기 없음`은 표시하지 않고, 합주가 있을 때만 `🎻 현악기` 태그 노출
- [x] done P2 2026-06-29 곡 목록 로딩 UI 개선: 흐릿한 회색 텍스트 대신 스피너와 명확한 상태 문구를 표시하는 공통 `LoadingState` 적용
- [x] done P1 2026-06-29 앵콜 정보 UI 강조: 곡 카드/추천 카드/상세 화면에 색상 배지로 앵콜 여부와 횟수를 명확히 표시
- [x] done P1 2026-06-29 라이브러리 앵콜곡 메뉴 추가: 정렬 옵션에 숨어 있던 앵콜곡 보기를 상단 `전체/앵콜곡` 세그먼트 메뉴로 분리
- [x] done P2 2026-06-29 하단 GNB 아이콘 개선: 기본 이모지를 `lucide-react` 선형 아이콘으로 교체
...
- 2026-06-29 앵콜 정보 UI 조정: 회색 텍스트로 묻히던 `앵콜 n회`를 재사용 가능한 배지로 분리하고, 라이브러리에서 앵콜곡만 보는 상단 메뉴를 추가했다. GNB는 이모지 대신 lucide 아이콘을 사용한다.
- 2026-06-29 로딩 상태 개선: 대시보드/라이브러리/곡 상세의 흐릿한 텍스트 로딩을 스피너, 테두리, 명확한 상태 설명이 있는 공통 컴포넌트로 교체했다.
- 2026-06-29 현악기 태그 표시 기준 변경: 현악기 없음은 기본 상태라 표시하지 않고, `strings=true`인 곡에만 `🎻 현악기` 배지를 표시한다.
- 2026-06-29 `할렐루야` 데이터 점검: Google Sheet `songs`에는 `할렐루야` 1곡(row 71, id `MF8Gk2Hdmck`)만 있고, `performances`에는 `2024-12-29`, `1부`, `new` 1행만 연결되어 있다. 공개 YouTube 플레이리스트 전체 183개 제목을 continuation까지 확인해도 `할렐/halle/알렐` 제목은 `2024 12 29 콰이어 할렐루야` 1건뿐이었다. 따라서 현재 낮은 횟수는 계산 버그보다 지정 플레이리스트 밖 과거 공연/다른 제목 영상 미포함 문제로 판단한다.
- 2026-06-29 추천 필터 확장: 기존 `derived.has2nd` 값을 사용해 `2부예배 찬양` 있음/없음 필터를 추가했다. 별도 DB 컬럼 없이 `performances.services`에 `2부`가 포함된 이력으로 판정한다.
