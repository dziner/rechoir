...
| --- | --- | --- | --- | --- |
| A | `id` | yes | `M7lc1UVf-VE` | 대표 YouTube video ID. 앱의 곡 primary key다. |
...
- 플레이리스트 동기화는 YouTube 제목과 설명에서 룰 기반 태그를 자동 추론한다. 성탄/부활/감사/찬양/경배/헌신/위로 같은 주제, 장엄/경건/서정/밝음/역동 분위기, 느림/빠름 템포, 난이도, 현악 여부를 키워드로 추정한다.
- 자동 태그는 수동 편집을 우선한다. `theme`/`mood`가 이미 있으면 유지하고, `tempo`/`difficulty`가 기본값 `mid`가 아닌 경우도 유지한다. `strings=false`는 기본값과 수동 해제를 구분할 수 없어, 제목/설명에 현악 키워드가 있으면 `true`로 보강될 수 있다. 따라서 정밀한 판정이 필요한 곡은 곡 상세 화면에서 직접 고친다.
- 이미 값이 들어간 `theme`/`mood`를 룰로 다시 채우고 싶으면 해당 셀을 비운 뒤 플레이리스트 동기화를 다시 실행한다.
- 룰 기반 자동 태그에는 추가 AI API 키가 필요 없다. 향후 더 정밀한 분류가 필요하면 YouTube 설명/자막 또는 LLM을 사용하는 별도 보강 작업으로 확장한다.
